...
(function (global, factory) {
    ...
    niceDatePicker.prototype.buildUi = function (year, month) {
        ...
        this.dayWords = [['M', 'T', 'W', 'T', 'F', 'S', 'S']];
        this.enMonthsWords = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'June', 'July', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];


        
        var html = '<div class="nice-date-picker-header">' +
            // '<div class="nice-date-picker-warpper">' +
            '<a href="javascript:;" class="prev-date-btn">&lt;</a>';

        if (!this.$params.mode) {
            this.$params.mode = 'zh';
            html += '<span class="nice-date-title">' + this.enMonthsWords[this.monthData.month - 1] + '&nbsp' + this.monthData.year + '</span>';
        } else if (this.$params.mode === 'en') {
            html += '<span class="nice-date-title">' + this.monthData.month + ' &nbsp ' + this.monthData.year + '</span>';
